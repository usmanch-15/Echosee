...
void main() {
  ...
  group('MainScreen BDD Tests -', () {
    testWidgets('Should show "Start Recording" initially',
    ...
    testWidgets(
        'Should toggle to "Stop Recording" and show live transcript header on tap',
        (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
    ...
    testWidgets('Stopping recording should call saveTranscript',
        (WidgetTester tester) async {
      registerFallbackValue(FakeTranscript());
      when(() => mockTranscript.saveTranscript(any()))
          .thenAnswer((_) async => {});
    ...
    testWidgets(
        'Should show red SnackBar if saveTranscript fails (Day 7 Scenario)',
        (WidgetTester tester) async {
      registerFallbackValue(FakeTranscript());
      when(() => mockTranscript.saveTranscript(any()))
          .thenThrow(Exception('Network Error'));
      when(() => mockTranscript.error)
      ...
      await tester.pump(const Duration(
          milliseconds: 500)); // Wait for snackbar to start showing

      expect(
          find.text(
              'Failed to save transcript: No internet connection. Please check your network.'),
          findsOneWidget);
